...
const { loadVecturaRuntime } = require('/private/tmp/claude-501/scratch-W32r4/tests/helpers/load-vectura-runtime');
...
(async()=>{
  ...
  rt.window.__SIL_PROTO_OFF=!proto;
  for(const mapper of ['hatch','contour','crosshatch','spiral','contourSlice']){
    for(const dens of [50,220]){
      ...
      out.push({proto,mapper,dens,backRuns:backFill.length,ends:ends.length,occluderCuts:cuts.length,
        ...
        gapMin:+(ds.length?Math.min(...ds):0).toFixed(4),gapMinPen:+((ds.length?Math.min(...ds):0)/PEN).toFixed(2),
        under0p5pen:ds.filter((d)=>d<-0.15).length,over0p5pen:ds.filter((d)=>d>0.15).length});
    }
  }
  }
