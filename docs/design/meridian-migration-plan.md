...
> **Phase 2 step 3 in progress (`5c2edcc` first extraction; 1 of 9 shell modules done):**
...
## Appendix: Resuming Phase 2 mid-flight (steps 1–2 done, step 3 in progress with 1 of 9 shell modules extracted)

Phase 2 is split into seven sequential steps. Steps 1 (CONTROL_DEFS extraction + compile gate, plus a companion toolbar fix) and 2 (`buildControls()` extraction into `algo-config-panel` + namespace-preservation patch + compile gate) landed in `a16ad57` + `313d427` + `7ca4795`. **Step 3 is in progress:** the first of 9 shell modules — `theme-switcher.js` — landed in `5c2edcc` as a proof-of-pattern beachhead. Eight shell modules remain. Five steps total remain.
...
2. `git log --oneline -10` — confirm the top commit is `5c2edcc` (theme-switcher first extraction) on top of `4cb6f04` (step 2 plan note), `7ca4795` (algo-config-panel), `5d3372f` (step 1 plan note), `313d427` (CONTROL_DEFS), `a16ad57` (toolbar fix). If you're somewhere else on the branch, stop and orient.
3. Read this file's "Phase 2 step 1 actuals" + "Phase 2 step 2 actuals" + "Phase 2 step 3 in progress" notes (immediately above the `### Phase 2:` heading) for what's already on disk. Then re-read §3 Phase 2 + §2.4 decomposition + §2.5 component API contract + §2.8 hidden DOM stash inventory (mandatory for the remaining shell modules — they touch the stashed DOM the legacy bindEvents code reads by id).
4. Confirm tests are green before changing anything: `npm run test:unit && npm run test:integration` (542 unit, 66 integration). The 12 compile-gate tests across `controls-registry-compile.test.js` (5) + `algo-config-panel-compile.test.js` (3) + `theme-switcher-compile.test.js` (4) are now baseline — must stay green through the rest of Phase 2.
5. **`window.Vectura.UI.{AlgoConfigPanel, CONTROL_DEFS, ThemeSwitcher}` are canonical.** All three are static properties on the legacy `UI` class itself (preserved by the namespace-copy patch in legacy `ui.js`). Don't reach back into legacy `ui.js`'s body copies of `buildControls` / `refreshThemeUi` — both are now 1-line delegators. The DI bags for both are bound near the bottom of the legacy IIFE (search for `AlgoConfigPanel.bind` and `ThemeSwitcher.bind`).
6. **Step 2's namespace-preservation pattern is now load-bearing AND proven across `panels/` + `shell/`.** Future shell/panel/component modules attached to `window.Vectura.UI.<Foo>` BEFORE `ui.js` loads survive the legacy `window.Vectura.UI = UI` assignment because `ui.js` copies prior static members forward. Whenever you add a new module that lives on `window.Vectura.UI.*`, it works automatically — no further patches needed. The `theme-switcher.js` extraction validated this for the `shell/` subtree.
7. **Phase 2 remaining moves**, in order:
   - **Step 3 — shell/ (CONTINUE).** First module `theme-switcher.js` ✅. Remaining 8: `pane-left`, `pane-right`, `workspace`, `header`, `menubar`, `toolbar`, `bottom-pane`, `shell`. Same `bind(deps)` DI-bag pattern, same compile-gate-test contract. Suggested next-smallest target: `menubar.js` (legacy lines ~7124-7260 of original `ui.js`, now shifted post-extractions — search current file for `setTopMenuOpen`, `triggerTopMenuAction`, `initTopMenuBar`). After menubar lands, pick the `pane-left` collapsible state extraction next, then batch the rest. Preserve every id in §2.8 (Hidden DOM Stash Inventory) — those ids are referenced by id from JS code that hasn't been extracted yet, so deletion or rename will silently break the legacy paths. **Note on testing matrix:** once the rest of step 3 lands, run `npm run test:visual` and `npm run test:perf` in addition to unit + integration before declaring step 3 done — shell DOM extraction warrants the visual + perf gate per CLAUDE.md.
