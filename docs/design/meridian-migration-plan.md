...
| **Phase 2** — Shell, panels, orchestrator | ⏳ in progress (steps 1–2 of 7 done) | `a16ad57` `313d427` `7ca4795` |
...
## Appendix: Resuming Phase 2 mid-flight (steps 1–2 done, step 3 next)

Phase 2 is split into seven sequential steps. Steps 1 (CONTROL_DEFS extraction + compile gate, plus a companion toolbar fix) and 2 (`buildControls()` extraction into `algo-config-panel` + namespace-preservation patch + compile gate) landed in `a16ad57` + `313d427` + `7ca4795`. Five steps remain.

After clearing context, the fastest way to pick up Phase 2 step 3:

1. `cd /Users/jayphi/Documents/github/vectura-studio-meridian` (the worktree, on branch `meridian-blue-skin`).
2. `git log --oneline -10` — confirm the top commit is `7ca4795` (algo-config-panel extraction) preceded by `5d3372f` (step 1 plan note), `313d427` (CONTROL_DEFS extraction), `a16ad57` (toolbar fix). If you're somewhere else on the branch, stop and orient.
3. Read this file's "Phase 2 step 1 actuals" + "Phase 2 step 2 actuals" notes (immediately above the `### Phase 2:` heading) for what's already on disk. Then re-read §3 Phase 2 + §2.4 decomposition + §2.5 component API contract + §2.8 hidden DOM stash inventory (mandatory for shell extraction).
4. Confirm tests are green before changing anything: `npm run test:unit && npm run test:integration` (538 unit, 66 integration). The 8 compile-gate tests across `controls-registry-compile.test.js` (5) + `algo-config-panel-compile.test.js` (3) are now baseline — must stay green through the rest of Phase 2.
5. **`window.Vectura.UI.AlgoConfigPanel` and `window.Vectura.UI.CONTROL_DEFS` are canonical.** Both are static properties on the legacy `UI` class itself (preserved by the namespace-copy patch in legacy `ui.js`). Don't reach back into legacy `ui.js`'s body copy of `buildControls` — it's now a 1-line delegator. The 40-key DI bag for algo-config-panel is bound at the bottom of the legacy IIFE (search for `AlgoConfigPanel.bind`).
6. **Step 2's namespace-preservation pattern is now load-bearing.** Future shell/panel/component modules attached to `window.Vectura.UI.<Foo>` BEFORE `ui.js` loads survive the legacy `window.Vectura.UI = UI` assignment because `ui.js` copies prior static members forward. Whenever you add a new module that lives on `window.Vectura.UI.*`, it works automatically — no further patches needed.
7. **Phase 2 remaining moves**, in order:
   - **Step 3 — shell/.** `pane-left`, `pane-right`, `workspace`, `header`, `menubar`, `toolbar`, `bottom-pane`, `theme-switcher`, `shell` (9 files). Mostly thin layout wrappers — should land fast now that `algo-config-panel` proves the extract-with-DI-bag pattern. Preserve every id in §2.8 (Hidden DOM Stash Inventory). Recommended approach: pick the simplest shell module first (e.g., `theme-switcher.js`), prove it can extract under the same `bind(deps)` pattern, then batch the rest. Each module gets its own compile-gate test mirroring `algo-config-panel-compile.test.js`.
