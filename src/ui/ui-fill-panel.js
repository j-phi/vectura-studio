...
(() => {
  ...
  const FILL_CAPS = {
    ...
    contour:     { angle: false, amplitude: false, dotSize: false, shift: false, contourDirection: true, contourStepVariance: true, contourSimplify: true, contourCenterPadding: true, contourBezier: true, contourSmoothing: true },
  ...
  const buildFillControlDefs = ({
    ...
    contourBezierParam        = 'fillContourBezier',
    contourSmoothingParam     = 'fillContourSmoothing',
    ...
    return [
      ...
      {
        id: contourBezierParam,
        label: 'Bezier Curves',
        type: 'checkbox',
        showIf: (p) => isActive(p) && !!caps(p).contourBezier,
        infoKey: `${descKeyPrefix}.contourBezier`,
      },
      {
        id: contourSmoothingParam,
        label: 'Smoothness',
        type: 'range',
        min: 0,
        max: 1,
        step: 0.05,
        showIf: (p) => isActive(p) && !!caps(p).contourSmoothing && p[contourBezierParam] !== false,
        infoKey: `${descKeyPrefix}.contourSmoothing`,
      },
      {
