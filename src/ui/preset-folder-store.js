...
(() => {
  ...
  const writePreset = async (system, slug, doc) => {
    ...
    try {
      ...
      // Signal the dev server to rebuild the preset bundle so the browser picks
      // up the new .vectura without a manual `npm run user-presets:bundle`.
      // Fire-and-forget: silently no-ops if the Node dev server isn't running.
      try {
        fetch('/api/bundle-presets', { method: 'POST' })
          .then((r) => { if (r.ok) window.dispatchEvent(new CustomEvent('vectura:presets-bundled')); })
          .catch(() => {});
      } catch (_) {}
      return true;
