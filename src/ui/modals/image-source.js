/**
 * Raster-Plane — source widget + paint modal.
 *
 * Installs three methods on UI.prototype:
 *   - mountImageSourceWidget(layer, wrapper)
 *       The inline panel control rendered for the rasterPlane `imageSource`
 ...
 * 'builtin') round-trip through `Vectura.RasterPlaneSource` so a saved project
 * reopens identically. See src/core/algorithms/raster-plane.js.
 */
(() => {
  ...
  const Source = () => Vectura.RasterPlaneSource;
  ...
  // The noise rack stack displaces the surface whenever an enabled layer exists
  // (each layer's own Field Weight drives the magnitude — there is no global
  // amount anymore). Gates the noise-aware preview render so the thumbnail tracks
  // the model the instant a layer is added.
  const noiseActive = (p) =>
    Array.isArray(p.noises) && p.noises.some((n) => n && n.enabled !== false && n.type !== 'imageSource');
  ...
  function applyImageSourceBlack(layer) {
    if (this.app && this.app.pushHistory) this.app.pushHistory();
    const p = layer.params;
    const store = (window.Vectura.NOISE_IMAGES = window.Vectura.NOISE_IMAGES || {});
    const id = 'imgsrc-solid-black';
    if (!store[id]) {
      const size = 4;
  ...
  function applyImageSourceWhite(layer) {
    if (this.app && this.app.pushHistory) this.app.pushHistory();
    const p = layer.params;
    const store = (window.Vectura.NOISE_IMAGES = window.Vectura.NOISE_IMAGES || {});
    const id = 'imgsrc-solid-white';
    if (!store[id]) {
      const size = 4;
      const data = new Uint8ClampedArray(size * size * 4).fill(255);
      store[id] = { width: size, height: size, data };
    }
    p.imageSourceKind = 'white';
    p.imageId = id;
    p.imageName = 'White';
    p.imageNoiseDef = null;
    p.imageSrc = '';
  ...
  // ---------------------------------------------------------------------------
  // Noise Preview widget (replaces the Source section — shows the combined
  // noise stack output with pop-out and paint button).
  // ---------------------------------------------------------------------------
  function mountNoisePreviewWidget(layer, wrapper) {
    const render = Render();
    const p = layer.params;
    const popout = this._imageSourcePopout || (this._imageSourcePopout = { open: false, x: null, y: null });
    ...
    const header = document.createElement('div');
    header.className = 'image-source-header';
    header.innerHTML = `<label class="control-label mb-0">Preview</label>`;
    const toggleBtn = document.createElement('button');
    toggleBtn.type = 'button';
    toggleBtn.className = 'image-source-popout-toggle';
    toggleBtn.title = popout.open ? 'Dock preview' : 'Pop out preview';
    toggleBtn.setAttribute('aria-label', toggleBtn.title);
    toggleBtn.innerHTML = popout.open ? DOCK_ICON : POPOUT_ICON;
    toggleBtn.onclick = () => {
      popout.open = !popout.open;
    ...
    const actions = document.createElement('div');
    actions.className = 'image-source-actions';
    actions.innerHTML = `<button type="button" class="image-source-btn is-paint">Paint</button>`;
    actions.querySelector('.is-paint').onclick = () => this.openImagePaintModal(layer);
    root.appendChild(actions);
    ...
    if (popout.open) {
      ...
      header.addEventListener('pointerdown', (e) => {
        ...
        const baseLeft = rect.left;
        const baseTop = rect.top;
        try { header.setPointerCapture(e.pointerId); } catch (_) { /* no-op */ }
        const move = (ev) => {
          const nx = Math.max(0, Math.min(window.innerWidth - 48, baseLeft + (ev.clientX - startX)));
          const ny = Math.max(0, Math.min(window.innerHeight - 32, baseTop + (ev.clientY - startY)));
          pane.style.left = `${nx}px`;
      ...
      const placeholder = document.createElement('div');
      placeholder.className = 'image-source-docked-placeholder mb-4';
      placeholder.innerHTML = '<span class="text-[11px] text-vectura-muted">Preview is floating</span>';
      const dockBtn = document.createElement('button');
      dockBtn.type = 'button';
      dockBtn.className = 'image-source-btn is-dock';
      dockBtn.textContent = 'Dock';
  ...
  // ---------------------------------------------------------------------------
  // Image Source picker for use inside the noise stack card.
  // Renders the source dropdown (Built-in → Black → White → Presets → Paint →
  // Import) and a file drop area that appears only when Import is active.
  // Updates layer.params directly (same path as the legacy widget).
  // ---------------------------------------------------------------------------
  function mountImageSourceInNoiseStack(layer, container) {
    const render = Render();
    const p = layer.params;
    const presets = Vectura.NOISE_IMAGE_PRESETS || [];

    const matchedPreset = () =>
      presets.find((preset) => p.imageNoiseDef && p.imageNoiseDef.type === preset.noise.type && p.imageName === preset.label) || null;

    const currentSourceLabel = () => {
      if (p.imageSourceKind === 'black') return 'Black';
      if (p.imageSourceKind === 'white') return 'White';
      if (p.imageSrc) return p.imageName || (p.imageSourceKind === 'painted' ? 'Painted' : 'Imported');
      const m = matchedPreset();
      if (m) return m.label;
      if (p.imageNoiseDef) return p.imageName || 'Noise';
    ...
    const makeSolidImageData = (r, g, b) => {
      const size = 28;
      const data = new Uint8ClampedArray(size * size * 4);
    ...
    const ddWrap = document.createElement('div');
    ddWrap.className = 'hg-preset-dropdown-wrap image-source-preset-dropdown mb-3';
    ...
    // Import drop/select area — shown only when Import is active or current kind is imported.
    const importArea = document.createElement('div');
    importArea.className = 'image-source-import-area mb-2';
    importArea.innerHTML = `
      <div class="noise-dropzone compact">Drop image</div>
      <button type="button" class="noise-image-btn text-xs border border-vectura-border px-2 py-1 hover:bg-vectura-border text-vectura-accent transition-colors">Select Image</button>
    `;
    const isImported = p.imageSourceKind === 'imported' || (p.imageSrc && p.imageSourceKind !== 'painted');
    importArea.classList.toggle('hidden', !isImported);
    ...
    const activePreset = matchedPreset();
    const isBuiltin = !p.imageNoiseDef && !p.imageSrc && p.imageSourceKind !== 'black' && p.imageSourceKind !== 'white' && p.imageSourceKind !== 'imported';
    makeOption('Built-in Relief', isBuiltin, () => (Source() && Source().renderBuiltinImageData ? Source().renderBuiltinImageData(64) : null), () => applyImageSourceBuiltin.call(this, layer));
    makeOption('Black', p.imageSourceKind === 'black', () => makeSolidImageData(0, 0, 0), () => applyImageSourceBlack.call(this, layer));
    makeOption('White', p.imageSourceKind === 'white', () => makeSolidImageData(255, 255, 255), () => applyImageSourceWhite.call(this, layer));
    presets.forEach((preset) => {
      makeOption(preset.label, activePreset === preset, () => presetThumb(preset), () => applyImageSourceNoise.call(this, layer, preset));
    });
    makeOption('Paint', p.imageSourceKind === 'painted', () => currentSourceImage(p) || null, () => this.openImagePaintModal(layer));
    makeOption('Import', p.imageSourceKind === 'imported', () => (p.imageSrc ? currentSourceImage(p) : null), () => { importArea.classList.remove('hidden'); });

    // Update trigger display.
    triggerLabel.textContent = currentSourceLabel();
    const srcImg = (() => {
      if (p.imageSourceKind === 'black') return makeSolidImageData(0, 0, 0);
      if (p.imageSourceKind === 'white') return makeSolidImageData(255, 255, 255);
      return currentSourceImage(p);
    })();
    if (srcImg && render) {
      triggerThumb.style.display = '';
      drawThumbTo(triggerThumb, srcImg);
    } else {
      triggerThumb.style.display = 'none';
    }

    // Wire up import area events.
    ...
    const dropzone = importArea.querySelector('.noise-dropzone');
    const selectBtn = importArea.querySelector('.noise-image-btn');
    if (dropzone) {
      dropzone.addEventListener('dragover', (e) => { e.preventDefault(); dropzone.classList.add('active'); });
      dropzone.addEventListener('dragleave', () => dropzone.classList.remove('active'));
      dropzone.addEventListener('drop', (e) => { e.preventDefault(); dropzone.classList.remove('active'); applyFile(e.dataTransfer?.files?.[0]); });
    }
    if (selectBtn) {
      ...
      fileInput.onchange = () => applyFile(fileInput.files?.[0]);
    }
  }
  ...
  Modals.ImageSource = {
    ...
    installOn(proto) {
      ...
      proto.applyImageSourceBlack = function (layer) {
        return applyImageSourceBlack.call(this, layer);
      };
      proto.applyImageSourceWhite = function (layer) {
        return applyImageSourceWhite.call(this, layer);
      };
      proto.mountNoisePreviewWidget = function (layer, wrapper) {
        return mountNoisePreviewWidget.call(this, layer, wrapper);
      };
      proto.mountImageSourceInNoiseStack = function (layer, container) {
        return mountImageSourceInNoiseStack.call(this, layer, container);
      };
    },
  };
})();
