/**
 * Vectura auto-colorize panel — Phase 3 closure: mixin dissolved.
 *
 * Owns the auto-colorization toggle, scope/mode selectors, parameter UI, and
 * the apply pipeline. Was previously split between:
 *   - src/ui/ui-auto-colorize.js (mixin attached to UI.prototype)
 *   - src/ui/panels/auto-colorize-panel.js (namespace anchor that forwarded
 *     to the mixin)
 *
 * Phase 3 step 6 dissolved the mixin into this module. The mixin file +
 * its `<script>` tag are removed; UI.prototype delegators (created in
 * legacy ui.js IIFE bind block) call directly into AutoColorizePanel.
 *
 * Public methods (all called as this.<method>(...) on UI instances via
 * the prototype delegators registered by legacy ui.js bind block):
 *   - initAutoColorizationPanel()
 *   - getAutoColorizationConfig()
 *   - getAutoColorizationTargets(scope)
 *   - applyAutoColorization(options)
 *
 * The bind(deps) DI bag now carries the legacy ui.js IIFE-locals that
 * the mixin used to read from its own IIFE: SETTINGS (live reference,
 * NOT a snapshot — the legacy bind block passes window.Vectura.SETTINGS
 * which is itself live), clamp, getEl. The mode tables stay private to
 * this module as they are not referenced anywhere else.
 *
 * Compile gate at tests/unit/auto-colorize-panel-compile.test.js.
 * Integration coverage at tests/integration/auto-colorize.test.js.
 */
(() => {
  ...
  // --- Module-private constants (formerly ui-auto-colorize.js IIFE locals). ---
  ...
  function initAutoColorizationPanel() {
    const { SETTINGS, getEl } = requireDeps('initAutoColorizationPanel');
  ...
  function applyAutoColorization(options = {}) {
    const { SETTINGS, clamp } = requireDeps('applyAutoColorization');
    const {
      commit = false,
      force = false,
  ...
  UI.AutoColorizePanel = {
    /**
     * Inject closure-captured legacy ui.js IIFE locals into the panel
     * after mixin dissolution. Idempotent — called once from the legacy
     * ui.js IIFE bind block.
     *
     * @param {object} deps
     * @param {object} deps.SETTINGS - live window.Vectura.SETTINGS reference
     * @param {function} deps.clamp - AlgorithmUtils.clamp
     * @param {function} deps.getEl - getElementById helper with warning
     */
    bind(deps) {
      DEPS = deps || {};
    },
    ...
    /**
     * Install the four panel methods on the supplied prototype (UI.prototype).
     * Replaces the old `Object.assign(UI.prototype, _UIAutoColorizeMixin)` call
     * in legacy ui.js. Each delegator invokes the panel function with `this`
     * bound to the UI instance — preserving the legacy mixin call shape.
     *
     * @param {object} proto - UI.prototype
     */
    installOn(proto) {
      proto.initAutoColorizationPanel = function(...args) {
        return initAutoColorizationPanel.apply(this, args);
      };
      proto.getAutoColorizationConfig = function(...args) {
        return getAutoColorizationConfig.apply(this, args);
      };
      proto.getAutoColorizationTargets = function(...args) {
        return getAutoColorizationTargets.apply(this, args);
      };
      proto.applyAutoColorization = function(...args) {
        return applyAutoColorization.apply(this, args);
      };
    },
  };
})();
