...
(() => {
  ...
  // The legacy `class UI { ... }` body has been stripped of every method.
  // What remained — the constructor + 28 prototype methods — moved out
  // in this unit:
  //   - Constructor body  → `src/ui/ui.js#init` (installed as `_init`
  //                          on UI.prototype via Orchestrator.installOn).
  //   - bindGlobal body   → `src/ui/ui.js#bindGlobalSweep` (private,
  //                          called from `init`).
  //   - getOptimizationTargets / getOptimizationTargetIds /
  //     optimizeTargetsForCurrentScope → `src/ui/modals/export-svg.js`.
  //   - recenterLayerIfNeeded, isDuplicateLayerName, getLayerById,
  //     isModifierLayer, getModifierState, assignLayersToParent,
  //     unlockMirrorChildrenOnDelete, shouldLeaveParentScope,
  //     isDescendant, normalizeGroupOrder, moveSelectedLayers,
  //     duplicateLayers, getLayerChildren, getLayerDescendants,
  //     canLayerAcceptChildren, getUniqueLayerName, renderLayers,
  //     expandLayer, getGroupDescendants → `src/ui/panels/layers-panel.js`.
  ...
  //
  // The class itself remains as a one-method stub: its sole responsibility
  // is to delegate construction into `this._init(app)`. Unit 1.10 will
  // delete this file entirely and move the IIFE-bootstrap block beneath
  // into a single ui.js initializer.
  class UI {
    constructor(app) {
      this._init(app);
    }
