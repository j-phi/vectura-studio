...
(() => {
  ...
  const create = (target, opts = {}) => {
    ...
    // When the dev-server finishes rebuilding user-presets.js after a folder
    // write, clear and re-inject the bundle so this gallery sees new params
    // without a page reload. Guards on target.isConnected so stale listeners
    // from replaced gallery instances are no-ops.
    window.addEventListener('vectura:presets-bundled', () => {
      if (!target.isConnected) return;
      const V = window.Vectura;
      if (V) V.PRESETS = [];
      const existing = document.querySelector('script[src*="user-presets.js"]');
      if (!existing) { refresh(); return; }
      const s = document.createElement('script');
      s.src = existing.src.split('?')[0] + '?t=' + Date.now();
      s.onload = () => { refresh(); toast('Preset library updated.', 'success'); };
      s.onerror = () => refresh();
