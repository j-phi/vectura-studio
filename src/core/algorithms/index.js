    ...
    topo: {
      generate: (p, rng, noise, bounds) => {
        ...
        for (let i = 0; i < rings; i++) {
          const ringOffset = (rng.nextFloat() - 0.5) * jitter;
          const rBase = Math.max(0.1, startR + i * gap + ringOffset);
          const steps = Math.max(64, Math.floor(rBase * 2));
          const path = [];
          for (let k = 0; k <= steps; k++) {
            const t = (k / steps) * Math.PI * 2 + angleOffset;
            const nx = Math.cos(t) * zoom + i * 0.12;
            const ny = Math.sin(t) * zoom + i * 0.12;
            const n = noiseValue(nx, ny);
            const wob = wobble ? Math.sin(t * 4 + i) * wobble : 0;
            const r = Math.max(0.1, rBase + n * amp + wob);
      ...
      formula: (p) =>
        `r = r0 + ${p.amplitude} * noise(θ*${p.zoom}) + wobble\nx = cx + cos(θ)*r, y = cy + sin(θ)*r`,
    },
    spiral: {
      generate: (p, rng, noise, bounds) => {
