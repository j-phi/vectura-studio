...
(() => {
  ...
  // Tessellation (grid/box meshing, pole welding) and the per-mode builder core
  // moved verbatim to Scene3D.Mesh; the samplers live in Scene3D.Charts. Param
  // resolution and the STL/importedMesh branch stay here: topoform hands the
  // resolved mode + sizes to Mesh.createTopoformMesh, which welds the raw grid
  // (UV pole de-fan + box edge sharing). An imported STL mesh is left as-is.
  const createPrimitiveMesh = (p, detail) => {
    ...
    return Vectura.Scene3D.Mesh.createTopoformMesh(mode, { sx, sy, sz }, detail);
  };
