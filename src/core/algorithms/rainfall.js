...
export const rainfall = {
      generate: (p, rng, noise, bounds) => {
        ...
        const fillDensity = Math.max(0.1, p.fillDensity ?? 1);
        ...
        const buildNoiseStack = () => {
                ...
                const applyEffect = (effect, state) => {
                  ...
                  switch (mode) {
                    ...
                    case 'edge': {
                      ...
                      const sy =
                        -sampleBlur(nextU - 1 / img.width, nextV - 1 / img.height, edgeBlur) -
                        2 * sampleBlur(nextU, nextV - 1 / img.width, edgeBlur) -
                        sampleBlur(nextU + 1 / img.width, nextV - 1 / img.height, edgeBlur) +
                        sampleBlur(nextU - 1 / img.width, nextV + 1 / img.height, edgeBlur) +
          ...
          return { noiseSamplers, maxAmp };
        };

        const { noiseSamplers, maxAmp } = buildNoiseStack();
        ...
        const applyNoiseOffset = (pt, dirX, dirY) => {
          ...
          const off = sampleNoise(pt.x, pt.y);
          const perpX = -dirY;
          const perpY = dirX;
          ...
          while (idx < total - 1) {
            ...
            switch (trailBreaks) {
              ...
              case 'burst':
                ...
                break;
              case 'drop':
                segLen = baseSeg * (1.5 + t * 2.5);
                gapLen = baseSeg * (0.6 + (1 - t) * 1.1);
                break;
              case 'drip':
        ...
        while (created < count && attempts < maxAttempts) {
          ...
          const trailUseNoise = noiseApply === 'trails' || noiseApply === 'both';
          const dropUseNoise = noiseApply === 'droplets' || noiseApply === 'both';
          const pathForBreaks = trailUseNoise ? basePath.map((pt) => applyNoiseOffset(pt, dirX, dirY)) : basePath;
          const broken = applyTrailBreaks(pathForBreaks, localStep);
          ...
          const dropBase = basePath[0];
          const dropCenter = dropUseNoise ? applyNoiseOffset(dropBase, dirX, dirY) : dropBase;
