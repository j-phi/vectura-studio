...
(function() {
  ...
  const pendula = (Array.isArray(PRESETS) ? PRESETS : [])
    .filter((preset) => preset?.preset_system === 'pendula');

  const PETALIS_LAYER_TYPES = new Set(['petalisDesigner']);
  const isPetalisLayerType = (type) => PETALIS_LAYER_TYPES.has(type);

  Vectura.PresetLibraries = { petalis, terrain, rings, harmonograph, pendula, PETALIS_LAYER_TYPES, isPetalisLayerType };
})();
