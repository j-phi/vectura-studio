...
- **User presets:** Place `.vectura` files in `user-presets/<layer_type>/` — the directory name must exactly match the layer's `type` field (= `preset_system`), including camelCase (e.g. `shapePack`, `svgDistort`, `petalisDesigner`). Run `npm run user-presets:bundle` to regenerate `src/config/user-presets.js`. When adding a new algorithm, create its `user-presets/<layer_type>/` directory (with a `.gitkeep`) at the same time.
