...
### Changed
- **Document Setup is now a centered modal on desktop (≥ 900 px viewports), with a live preview of the document.** File ▸ Document Setup and `Cmd/Ctrl+K` open a wide modal whose left pane shows a live snapshot of the canvas (refreshed two animation frames after every form change) alongside a paper-size + margin readout, while the right pane hosts the same form body that powers the drawer. The form body is *moved* between the drawer and the modal rather than duplicated, so the ~30 `#set-*` ids stay unique and the legacy `bindGlobal` handlers fire wherever the form lives. On viewports below 900 px the slide-out drawer remains the surface — the threshold matches `body.mobile-layout` in `workspace.js` so the two layout signals can never disagree.
