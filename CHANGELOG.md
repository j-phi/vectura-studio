...
### Added
- **Pendula oscillator cards — pluck pad + per-parameter dice padlocks.** Each pendulum card now leads with a **pluck pad**: a drag-vector control where drag length sets release amplitude (written to both ampX and ampY) and drag angle sets release direction (both phaseX and phaseY) — the "release is the performance" gesture from real harmonographs, in one move. The numeric ampX/ampY/phaseX/phaseY inputs move into an **Advanced** disclosure on the card (still the editable source of truth; editing them moves the pad handle), while Frequency/Micro/Damping stay visible. Every per-pendulum control gains a **padlock**: locked params (stored in `layer.params.pendulumParamLocks[id][key]`, serialized for free) are skipped by Dice/Mutate, so you can freeze what you like and reroll the rest. The pad is DPR-scaled and pushes one undo entry per drag. (RGR: `pendula-oscillator-controls.test.js` — pad drag sets equal amp/phase + commits with a single history push, the four numerics live in the Advanced disclosure, and a locked freq survives a dice roll while unlocked params change.)
