...
- Petal Designer inline editor and pop-out window use high-DPI rendering with immediate canvas updates, target switching (`Inner`/`Outer`, default `Inner`), per-target profile resets, symmetry modes (`None`, `Horizontal`, `Vertical`, `Horizontal and Vertical`), default dual-ring controls (`Inner Ring Count`/`Outer Ring Count`), `Split Feathering`, and a full shading stack embedded in the designer panel. The inline panel can pop out (⧉) and pop back in (↩) while keeping the exact same controls and layout.
...
7. Switch to the Petalis Designer algorithm to use the embedded inline designer panel, then use ⧉ to pop it out into a floating window or ↩ to dock it back in. In Petalis Designer, petal shape is driven by the visible inner/outer designer curves plus inline ring-count/split-feather/shading/symmetry controls (the Petal Geometry `Petal Profile` dropdown and legacy hidden tip/base shape modifiers are not used).
