...
- Petal Designer inline editor and pop-out window use high-DPI rendering with immediate canvas updates, target switching (`Inner`/`Outer`, default `Inner`), per-target profile resets, symmetry modes (`None`, `Horizontal`, `Vertical`, `Horizontal and Vertical`), default dual-ring controls (`Inner Petal Count`/`Outer Petal Count`, 20 each by default), `Split Feathering`, and a full shading stack embedded in the designer panel. The inline panel can pop out (⧉) and pop back in (↩) while keeping the exact same controls and layout.
...
7. Switch to the Petalis Designer algorithm to use the embedded inline designer panel, then use ⧉ to pop it out into a floating window or ↩ to dock it back in. In Petalis Designer, petal shape is driven by the visible inner/outer designer curves plus inline inner/outer petal-count, split-feather, shading, and symmetry controls (the Petal Geometry `Petal Profile` dropdown and legacy hidden tip/base shape modifiers are not used).
...
- Petalis Designer: Petalis variant with embedded interactive petal-shape and shading-stack controls plus default dual-ring count-driven transition and split-feather/symmetry controls (defaults: `radialGrowth` 0.05), with shape controlled by visible designer curves instead of hidden legacy tip/base modifiers.
