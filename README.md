...
- Petal Designer inline editor and pop-out window use high-DPI rendering with immediate canvas updates, target switching (`Outer`/`Inner`), per-target profile resets, symmetry modes (`None`, `Horizontal`, `Vertical`, `Horizontal and Vertical`), default dual-ring controls (`Inner Ring Count`/`Outer Ring Count`), `Ring Split`, `Split Feathering`, and a full shading stack embedded in the designer panel. The inline panel can pop out (⧉) and pop back in (↩) while keeping the exact same controls and layout.
