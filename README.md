...
Wallpaper mirrors open in a gallery-first **Styles** mode: a grid of cards, each a live thumbnail of *your own geometry* tiled under that symmetry. Cards cover all 17 groups (in plain language) plus curated named recipes (Windowpane, Tatami, Honeycomb, Op-Art Weave, Snowflake Lace, Kasbah Tile, …) — click one to apply it. A **Surprise me** dice rolls a random valid group with tasteful tile parameters (Shift-click locks the lattice family). On the canvas, drag the **center puck** to move the symmetry center and the **rotate ring** to spin the whole lattice (Shift snaps).
