...
| **3D Spiral** | Lines or dot loops wrapped around sphere/cone/cylinder/ellipsoid/torus/capsule surfaces with front-only or see-through projection, curve smoothing, and orthographic or perspective view |
...
### 1.1.112
- **Image Surface "Curves" now actually curves the lines — live, the moment you flip the toggle.** Turning on the layer **Curves** toggle for an Image Surface used to do nothing (and made the smoothing slider look like the real on/off switch). Curves now bends every point into a smooth bézier across **Relief Lines**, **Deformed Mesh**, and **Topography** as soon as you tick it, and the **Curve Smoothing** slider (renamed from *Contour Smoothing*, shown while Curves is on) is a real gradient — slide it up for progressively smoother, leaner curves. Bars are unaffected, and existing files render exactly as before until you flip Curves on.
