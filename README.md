...
- Petal Designer inline editor and pop-out window use high-DPI rendering with immediate canvas updates, a `Petal Ring` selector (`Inner`/`Outer`/`Both`) that drives ring generation and count visibility, symmetry modes (`None`, `Horizontal`, `Vertical`, `Horizontal and Vertical`), default dual-ring controls (`Inner Petal Count`/`Outer Petal Count`, 20 each by default), `Split Feathering` in `Both` mode, and targeted shading stacks so each ring can have its own stack (with optional shared `Both` stack). The inline panel can pop out (⧉) and pop back in (↩) while keeping the exact same controls and layout.
...
- Petalis Designer: Petalis variant with embedded interactive petal-shape controls, `Petal Ring` targeting (`Inner`/`Outer`/`Both`), per-ring shading stacks, and count-driven transition/split-feather/symmetry controls (defaults: `radialGrowth` 0.05), with shape controlled by visible designer curves instead of hidden legacy tip/base modifiers.
