/**
 * Regression test for audit defect D2: topoform `artworkSize` is DEAD.
 *
 ...
 *  2. `ALGO_DEFAULTS.topoform.artworkSize` is `undefined` after removal.
 */
const { loadVecturaRuntime } = require('../helpers/load-vectura-runtime');

describe('Topoform — artworkSize removed (audit D2, dead/redundant)', () => {
  let runtime;
  ...
  it('ALGO_DEFAULTS.topoform.artworkSize is undefined (key removed)', () => {
    const { ALGO_DEFAULTS } = runtime.window.Vectura;
    expect(ALGO_DEFAULTS.topoform.artworkSize).toBeUndefined();
  });

  it('rasterPlane.artworkSize is left intact (real parameter, not removed)', () => {
    const { ALGO_DEFAULTS } = runtime.window.Vectura;
    expect(ALGO_DEFAULTS.rasterPlane.artworkSize).toBe(150);
  });
});
