/*
 * Raster-Plane source pipeline — RGR coverage (v1.1.97).
 *
 ...
 *   - RasterPlaneSource.ensure populates NOISE_IMAGES from a noise descriptor
 *     and is cached/idempotent.
 *   - rasterPlane.generate samples that source — a noise source yields
...
describe('Raster-Plane — source pipeline', () => {
  let runtime;
  let V;
  ...
  describe('RasterPlaneSource.ensure', () => {
    ...
    test('returns false for a built-in source (no raster needed)', () => {
      ...
      expect(V.RasterPlaneSource.ensure(p)).toBe(false);
      expect(p.imageId).toBeFalsy();
    });
  });

  describe('rasterPlane.generate samples the source', () => {
    test('a noise source yields different geometry than the built-in relief', () => {
      const algo = V.AlgorithmRegistry.rasterPlane;
