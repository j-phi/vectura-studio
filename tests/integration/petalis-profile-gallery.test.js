...
describe('Petalis visual profile gallery', () => {
  ...
  test('the designer panel omits the redundant profile gallery (it lives in the inline designer)', () => {
    // The named-profile picker moved INTO the Petal Designer (its own per-side
    // thumbnail gallery), so the duplicate panel "Petal Geometry" gallery is stripped.
    const defs = window.Vectura.UI.CONTROL_DEFS.petalisDesigner;
    expect(galleryDef(defs)).toBeUndefined();
    expect(defs.some((d) => d && d.type === 'petalDesignerInline')).toBe(true);
  });
